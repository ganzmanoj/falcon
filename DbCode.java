...
public class DbCode extends JFrame implements ActionListener
{	static String user;
	...
	{	
		try
		{
                        ...
                        String sql=("insert into signup values('"+a+"','"+b+"','"+c+"','"+d+"','"+e+"','"+f+"','"+g+"','"+h+"')");
                        System.out.println(sql);
			Class.forName("com.mysql.jdbc.Driver");
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/blue","root","jonam");
                        st1=con.createStatement();
			st1.executeUpdate(sql);
                        rs1=st1.executeQuery("select * from signup");
                        while(rs1.next())
                        {
	...
	DbCode(double a)
	{
                 try
		{
                        ...
                        String sql=("select avatar from signup where username='"+user+"'");
	...
	DbCode(String a,double b)//avatar
	{
		...
		{       
			String updateString1;
                        updateString1 = ("update signup set avatar = '"+a+"' where username = '"+user+"'");
...
DbCode(String a,String b,String c)
	{
		try
		{           
                        String sql=("select * from signup where username='"+a+"' and forgot='"+c+"'and answer='"+b+"'");
                        Class.forName("com.mysql.jdbc.Driver");
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/blue","root","jonam");
                        st1=con.createStatement(); 
                        rs1=st1.executeQuery(sql);
...
DbCode(String a)
	{
		...
		{
			String updateString1;
                        updateString1 = ("update signup set name = '"+a+"' where username = '"+user+"'");
...
DbCode(String a,int b)
	{
		...
		{
			String updateString1;
                        updateString1 = ("update signup set password = '"+a+"' where username = '"+user+"'");
                        Class.forName("com.mysql.jdbc.Driver");
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/blue","root","jonam");
                        st1=con.createStatement();
			st1.executeUpdate(updateString1);
...
DbCode(String a,String b,int c)
	{
		...
		{
			String updateString1;
                        updateString1 = ("update signup set forgot = '"+a+"',answer = '"+b+"' where username = '"+user+"'");
			//String updateString2 = ("update signup set answer = '"+b+"' where username = '"+user+"'");
                        Class.forName("com.mysql.jdbc.Driver");
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/blue","root","jonam");
                        st1=con.createStatement();
			st1.executeUpdate(updateString1);
...
DbCode(String a,String b,String c,int d)
	{
		try
		{
			...
			con=DriverManager.getConnection("jdbc:mysql://localhost:3306/blue","root","jonam");
                        st1=con.createStatement();
			String sql2=("select * from signup where username='"+a+"'");
  			...
  			while (res.next())
					 {
  					String from1 = res.getString("from1");
							...
							frame.setSize(700,700);
								...
								JLabel l1=new JLabel("TO");
							        JLabel l2=new JLabel("SUBJECT");
                                                                l1.setBounds(60,40,150,20);
								l2.setBounds(200,40,150,20);
								Background bag = new Background(new ImageIcon("images/dragon.jpg").getImage());//to set background
								JButton back=new JButton("Back");
								back.setBounds(430,550,100,30);		
                                                                 frame.add(l1);
								frame.add(l2);
								frame.add(back);
							...
							back.addActionListener(this);
								...
								j[i][1].setBounds(x+150,y,400,30);
